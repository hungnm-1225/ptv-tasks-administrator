...
export const GitCollaboratorTab: React.FC<GitCollaboratorTabProps> = ({
    ...
    // Chuẩn hóa 3 trạng thái: 'add' | 'remove' | 'jit'
    ...
    // =========================================================================
    // 📊 BỘ BÓC TÁCH DỮ LIỆU GOOGLE SHEETS TỰ ĐỘNG TỪ gitUsersList
    // =========================================================================
    const parsedJitAccounts = useMemo<ParsedJitAccount[]>(() => {
        if (!isJitMode || !gitUsersList.trim()) return [];
        const lines = gitUsersList.split(/\r?\n/).map((l) => l.trim()).filter(Boolean);
        const results: ParsedJitAccount[] = [];

        lines.forEach((line, index) => {
            const lower = line.toLowerCase();
            // Tự động bỏ qua dòng tiêu đề nếu copy dính header
            if (
                index === 0 &&
    ...
    return (
        ...
        >
            ...
            {/* BẬT/TẮT CHUYỂN ĐỔI 3 HÀNH ĐỘNG */}
            ...
            {/* ================================================================= */}
            {/* 🎯 GIAO DIỆN JIT: CHỈ HIỆN Ô DÁN SHEET & BẢNG PREVIEW              */}
            {/* ================================================================= */}
            {isJitMode ? (
                <div className="space-y-4 animate-in fade-in duration-200">
                    <div className="p-4 rounded-2xl bg-amber-50/70 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900/60 flex items-start gap-3">
                        ...
                        <div className="text-xs text-amber-900 dark:text-amber-200 space-y-1">
                            ...
                            <p className="text-[11px] leading-relaxed text-amber-800/90 dark:text-amber-300/80">
                                Copy 2 cột <strong>Tài khoản</strong> và <strong>Mật khẩu</strong> từ Google Sheet hoặc Excel rồi dán vào ô bên dưới. Bấm nút tím <strong>Kiểm Tra & Kích Hoạt Worker Chạy Ngay</strong> ở cuối trang để hệ thống khởi tạo tài khoản ngầm (~300ms/user)!
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="space-y-1.5">
                        ...
                        <textarea
                            ...
                            placeholder={"teachersabah8\tteacher_sabah8@edu.my\nteachersabah9\tteacher_sabah9@edu.my"}
                    ...
                    {/* BẢNG LIVE PREVIEW BENTO GRID (KHÔNG CÒN NÚT CAM THỪA THÃI) */}
                ...
                <div className="space-y-4">
                    ...
                    {/* Danh sách User cho luồng Thêm/Gỡ */}
                    <div className="space-y-1.5">
                        <div className="flex items-center justify-between text-xs font-semibold text-slate-700 dark:text-slate-300">
                            <span>
                                {isRemoveMode
