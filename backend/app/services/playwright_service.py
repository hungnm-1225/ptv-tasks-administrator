...
# Bộ cờ tối ưu hóa siêu tiết kiệm RAM dành riêng cho môi trường Render 512MB RAM
LOW_MEMORY_CHROMIUM_ARGS = [
    ...
    "--js-flags=--max-old-space-size=128",  # Khóa trần V8 Heap ở mức 128MB ngăn chặn triệt để OOM
]
...
class PlaywrightLMSService:
    ...
    - Bảo vệ bộ nhớ máy chủ Render 512MB RAM với Concurrency Lock & Resource Aborting.
    ...
    def __init__(self):
        self.headless = True
        # Khóa đơn tiến trình: Ngăn chặn 2 browser chạy song song gây nổ RAM
    ...
    async def _handle_route_abort(self, route: Route):
        """Chặn tải hình ảnh, font chữ, media và analytics để tiết kiệm tối đa 70% RAM."""
        req = route.request
        res_type = req.resource_type
        url = req.url.lower()

        if res_type in ["image", "media", "font"] or any(
            url.endswith(ext) for ext in [
                ".png", ".jpg", ".jpeg", ".gif", ".webp", ".ico", ".svg",
                ".woff", ".woff2", ".ttf", ".eot", ".otf", ".mp4", ".mp3"
            ]
        ) or "google-analytics" in url or "doubleclick" in url:
            await route.abort()
        else:
    ...
    async def _login_moodle_sso(self, page: Page) -> bool:
        ...
        try:
            ...
            # Sử dụng domcontentloaded thay vì load để tránh kẹt mạng và giữ RAM
    ...
    async def enroll_users_pipeline(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Bọc khóa Concurrency Lock ngăn 2 task chạy song song gây OOM
        async with self._lock:
            ...
            async with async_playwright() as p:
                ...
                # Kích hoạt bộ lọc chặn triệt để hình ảnh, media, font
                ...
                try:
                    ...
                    await browser.close()
                    # Thu hồi bộ nhớ RAM nhị phân ngay lập tức
                    gc.collect()
