...
class WorkspaceOrchestratorService(WorkspaceOrderService, WorkspaceContractService, WorkspaceEnrollService):
    ...
    Nâng cấp lên Kiến trúc Step Engine có Checkpoint 2.0 & State Machine:
    ...
    async def execute_full_license_hierarchy_chain(
        ...
        Bỏ qua 100% các bước đã hoàn tất khi Resume.
        """
        ...
        log_step(f"🚀 [TRỌN GÓI 4-IN-1] Bắt đầu quy trình (State Machine Checkpoint 2.0) cho: '{school_identifier}'")
        ...
        if "drive_upload" in completed and drive_link:
            ...
            try:
                ...
                log_step(f"⚠️ [BƯỚC 1 - DRIVE] Lỗi tải Drive (không chặn tiến trình): {e}")
        ...
        # ------------------------------------------------------------------
        # BƯỚC 2: SCHOOL TẠO ORDER (IDEMPOTENCY GUARD)
        # ------------------------------------------------------------------
        ...
        if "school_create_order" in completed and order_code:
            log_step(f"⏩ [BƯỚC 2 - SCHOOL] Order Code đã tồn tại trong Checkpoint: [{order_code}]. Bỏ qua tạo mới!")
        else:
        ...
        # ------------------------------------------------------------------
        # BƯỚC 3: PHÂN PHỐI VÀ DUYỆT LICENSE (BOOMERANG CASCADE)
        # ------------------------------------------------------------------
        cp["current_step"] = "license_cascade"
        if "license_cascade" in completed:
            log_step(f"⏩ [BƯỚC 3 - LICENSE] Order [{order_code}] đã được cấp phép License hoàn tất. Bỏ qua!")
        else:
            log_step(f"🤝 [BƯỚC 3 - LICENSE] Kích hoạt chuỗi phê duyệt License Boomerang cho [{order_code}]...")
        ...
        # ------------------------------------------------------------------
        cp["current_step"] = "school_enroll_users"
        student_emails = order_details.get("student_emails", [])

        if "school_enroll_users" in completed:
            ...
            completed.append("school_enroll_users")
            resources["group_name"] = enroll_res.get("group_name")
    ...
    async def execute_approve_school_order_standalone(
        ...
        if not courses_needed:
            ...
            if courses_needed:
                log_step(f"📚 Đã bóc tách được {len(courses_needed)} môn học từ Order.")
        ...
        if partner_res.get("status") == "insufficient_pool_created_prt":
            ...
            # 4. Kích hoạt Distributor (và Sales Admin nếu cần) duyệt PRT Contract
            ...
            log_step(f"🎉 [BOOMERANG CHIỀU VỀ] Cấp bù thành công! Partner duyệt lại School Order [{order_identifier}]...")

            # 5. Chiếc Boomerang quay lại đích duyệt dứt điểm
    ...
    async def execute_approve_partner_contract_standalone(
        ...
        """Duyệt Partner Contract 1-Session kèm lưu vết Checkpoint 2.0."""
        cp = normalize_checkpoint_v2(checkpoint)
        resources = cp["resources"]
        completed = cp["completed_steps"]
        logs = []
