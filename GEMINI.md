...
|---|---|---|---|
| **1** | `frontend-specialist` | [frontend-specialist.md](file:///c:/Users/dtt/Desktop/Project/ptv-tasks-administrator/.agent/agents/frontend-specialist.md) | React 19, TypeScript Strict, Tailwind CSS v4, Bento Grid, Enterprise Pastel OKLCH, Dark/Light theme, Evidence Provenance UI, loại bỏ nhãn song ngữ thừa, responsive 14 trang, SheetJS Excel Preview đa tab modal, Automation Studio 4 Engine Tabs (Kiến trúc Module 13 tệp con, 5 Workspace Sections bao gồm `update_user`), Hierarchy Manager Portal Modals & Pagination. |
| **2** | `backend-specialist` | [backend-specialist.md](file:///c:/Users/dtt/Desktop/Project/ptv-tasks-administrator/.agent/agents/backend-specialist.md) | Python 3.11/3.12, FastAPI 0.115, Pydantic v2 validation, Async/Await, Dual-Key Gemini cross-failover với AI-Summary Context Bridging, Deterministic Fast-Path Triage, True Topological Sort (Kahn), Safe Job Wrapper, Ma trận 8 RAM Caches 1ms, Hybrid RPA-API Architecture với Ephemeral Session Caching, Unified Session Keepalive v2.0 (`session_keepalive_service.py`), 7 Crons so le (+15s đến +900s), Circuit Breaker pattern, Multi-Course Git Sync, 5-in-1 Master Orchestrator, Workspace User Profile Engine (`user_service.py`). |
...
### 2.1. Cấu Trúc 7 Phân Hệ Pythaverse & Kiến Trúc Vận Hành Hybrid RPA-API (V4.0)
> **Triết lý Hybrid RPA-API kết hợp Ephemeral Session Caching & Unified Keepalive:**
...
> - **Async Non-blocking HTTP Engine (HTTPX 150–300ms):** Toàn bộ nghiệp vụ nặng nề (Tạo đơn hàng Order, Ký hợp đồng Contract, Nộp batch tài khoản, Cron polling, Ghi danh/hủy ghi danh đa môn học Moodle, Phân quyền GitBucket, Tra cứu & Cập nhật User Profile) được thực thi 100% qua các HTTP POST/REST WebService ngầm siêu tốc, nhanh hơn 10–50 lần so với việc click UI giả lập.
> - **Unified Session Keepalive v2.0 (`session_keepalive_service.py`):** Duy trì "nóng" các phiên đăng nhập quản trị qua HTTPX lightweight ping ngầm (15 phút/lần), tuần tự gieo mầm (sequential seeding) qua Semaphore để không xung đột tài nguyên, hỗ trợ endpoint làm nóng tập trung `POST /api/v1/workspace/prewarm-all-sessions`.
...
1. **School Workspace (`pythaverse.space`):** Quản trị phân cấp 3 tầng (`Distributor` ➔ `Partner` ➔ `School`). School tạo Order lên Partner; Partner cấp License từ Pool; Distributor duyệt Contract cấp bù; Sales Admin phê duyệt tối cao.  
   - **Workspace Fast Engine V4.0:** Playwright đăng nhập trích xuất Cookie + `window.user` (WordPress identity: `school_id`, `partner_id`, `distributor_id`, `user_id`) trong 3s, sau đó đóng trình duyệt và gửi trực tiếp HTTPX Multipart POST tới các endpoint PHP (`schoolCreateOrder.php`, `updateStatusOrder.php`, `createOrderSale.php`, `updateStatusPartnerOrder.php`, `createOrder.php`, `/wp-json/...`).
...
2. **PLearn LMS (`learn.pythaverse.space`):** Moodle LMS (PHP / MariaDB / Edwiser RemUI). Quản lý danh mục khóa học (`SWRP`, `IR`, `ASP`...) và ghi danh/hủy ghi danh theo Role (`Student` - 9, `Teacher` - 7, `Manager` - 1).  
   - **LMS Fast Engine Hybrid V4.0:** Playwright SSO Keycloak (3s) trích xuất Cookie & sesskey ➔ HTTPX Direct WebService (`core_enrol_manual_enrol_users`, `core_enrol_unenrol_user_enrolment`, `core_user_get_users_by_field`) với `MOODLE_SEARCH_SEMAPHORE = 10` song song. Fallback 2 nhịp trên `td.cell.c2` khi WebService bị khóa. Hỗ trợ đa dạng cấu trúc đầu vào (`list[dict]`, `list[str]`, `list[int]`).
3. **PGit Repos (`git.pythaverse.space`):** Máy chủ GitBucket (Scala/JVM). Yêu cầu đăng nhập SSO qua Keycloak ít nhất 1 lần để kích hoạt tài khoản JIT.  
   - **Git Fast Engine Hybrid V4.0:** Playwright SSO Session Stealer (3s) trích xuất Cookie `GITBUCKET_SESSION_ID`, kiểm tra tài khoản đã đăng nhập JIT hay chưa qua `POST /_user/existence` (chỉ mất 20ms). Nếu tồn tại, gửi trực tiếp `POST /settings/collaborators` (200ms). Người dùng chưa đăng nhập được gom vào danh sách `not_logged_in_git` cảnh báo cho Admin mà không làm dừng cả pipeline.
   - **2-Vector DOM Parser & Bot Admin Protection:** Khi bóc tách danh sách cộng tác viên từ trang `/settings/collaborators`, Vector 1 đọc qua nhãn radio button đang chọn; Vector 2 quét các thẻ neo `a` có nút `(remove)` liền kề. Tài khoản quản trị bot (`admin_user`) được bảo vệ vĩnh viễn (hardcoded), không bao giờ bị xóa nhầm và luôn duy trì quyền `ADMIN`.
4. **Keycloak Auth IDP (`eid.pythaverse.space`):** Cổng xác thực tập trung OAuth2/OIDC (Realm: `master` / `idp`). 2-Tier Hybrid: Direct Admin REST API (300ms) với `KEYCLOAK_SEMAPHORE = 10` + In-Memory Token Cache ➔ Playwright RPA Fallback. Cập nhật trạng thái tài khoản kích hoạt/vô hiệu hóa hỗ trợ cờ `enabled` trực tiếp và các bí danh `lock`, `disable`, `unlock`, `enable`.
...
### 2.2. Kiến Trúc Dual-Path AI Cognition, AI-Summary Context Bridging & Fast-Path Triage
- **Tách Biệt Nhận Thức Kép (Dual-Path Cognition):**
  - *Luồng Nhận Thức Mềm (`summarize_ticket` - Key 1):* Tóm tắt nội dung, phân loại danh mục (`category`) và độ ưu tiên (`priority`) để hiển thị trực quan trên Unified Inbox cho con người đọc. **Không sinh action hay thay đổi dữ liệu.**
  - *Luồng Vận Hành Xác Định (`extract_operational_facts` - Key 2):* Trích xuất Ý định (Intents) và Thực thể (Entities) bị khóa chặt bởi **Trích dẫn bằng chứng nguyên văn (`evidence_quotes`)** và định vị ký tự.
  - 🌟 **Cơ Chế AI-Summary Context Bridging:** Bản tóm tắt tiếng Việt `summary_vi` từ Luồng Mềm được tự động truyền làm ngữ cảnh dẫn đường (`[BẢN TÓM TẮT Ý ĐỊNH ĐÃ TINH CHẾ TỪ HỆ THỐNG]`) cho Luồng Bóc Tách Sự Thật ở Key 2, giúp Gemini trích xuất chính xác 100% ý định cốt lõi và mở rộng toàn diện dải môn học.
...
- **Tự Động Phân Giải Khóa Học & Đồng Bộ Git Repos (`resolve_course_and_repos_from_db`):**
  - Tự động nhận diện tên môn học, tra cứu bảng `lms_courses` / `workspace_courses`.
  - Tự động lấy cấu hình Git Repositories liên kết và tích hợp Git Sync vào bước thực thi.
...
### 2.6. Cơ Sở Dữ Liệu 23 Bảng CSDL Supabase & Stored Procedures
...
22. `workspace_active_sessions`: Bảng lưu trữ phiên đăng nhập tái sử dụng có khóa mã hóa Fernet, thời gian hết hạn và cờ kiểm tra hiệu lực.
23. `cron_telemetry_state`: Bảng lưu trữ trạng thái vi lượng telemetry của 7 background cronjobs ngầm (thời gian chạy, kết quả, độ trễ, số lượng bản ghi xử lý).
- **2 Stored Procedures Bất Biến & RPC Fallback:**
  ...
  - *Fallback Table Update:* Nếu môi trường CSDL chưa kịp apply stored procedure, `workflows.py` tự động chuyển sang Fallback Direct Table Update kèm ghi nhận execution event nguyên tử mà không làm gián đoạn phê duyệt.
...
4. **Dual-Freeze Proposal & Immutable Provenance Linkage:**
   ...
   - **Admin Override & Dual Sync:** Khi Quản trị viên tinh chỉnh các bước trên giao diện, hệ thống tự động đồng bộ cả Workflow lẫn Proposal sang trạng thái `ready_for_review` và cho phép phê duyệt thực thi kể cả khi ban đầu là `needs_information`.
...
6. **Optimistic Concurrency Control (OCC) Lease, Single-Instance Concurrency & Circuit Breaker (Render 512MB RAM):**
   ...
   - **Chốt Chặn Circuit Breaker (`is_heavy_operation_running` & `heavy_operation_guard`):** Khi hệ thống đang thực thi thao tác VIP nặng, context manager `heavy_operation_guard` kéo cờ ưu tiên. Toàn bộ 7 cronjobs ngầm tự động nhận diện và tạm hoãn lượt chạy (`[CircuitBreaker] Cronjob tạm hoãn`), triệt tiêu 100% nguy cơ OOM Kill của Render.
   - Mọi coroutine chạy Playwright hoặc quét ngầm bắt buộc gọi `gc.collect()` và `force_kill_zombie_chromium()` trong khối `finally`.
   - **7 Crons trong `main.py` xuất phát lệch pha so le (Staggered Startup):**
     1. `keepalive_cron`: +15s startup delay, chu kỳ 15 phút (900s).
     2. `site_uptime_cron`: +30s startup delay, chu kỳ 5 phút (300s).
     3. `gmail_cron`: +45s startup delay, chu kỳ 5 phút (300s).
     4. `sheet_cron`: +75s startup delay, chu kỳ 5 phút (300s).
     5. `workspace_long_tasks_cron`: +150s startup delay, chu kỳ 5 phút (300s).
     6. `osticket_cron`: +240s startup delay, chu kỳ 10 phút (600s).
     7. `distributor_cache_scanner_cron`: +900s startup delay, chu kỳ 45 phút (2700s).
...
1. [`workflows.py`](file:///c:/Users/dtt/Desktop/Project/ptv-tasks-administrator/backend/app/api/v1/endpoints/workflows.py): Quản trị Workflow drafts, Server-side JWT approval gate v4.1 (`POST /{id}/approve_and_run`), Admin Override, Dual Sync, Dual Freeze, RPC Fallback, Retry steps (`POST /{id}/steps/{step_id}/retry`), Graph validation (`POST /{id}/validate`), Cancel workflow (`POST /{id}/cancel`).
...
### 4.2. Các Dịch Vụ Chủ Chốt Backend (`backend/app/services/` & `backend/app/core/`)
...
10. [`keycloak_service.py`](file:///c:/Users/dtt/Desktop/Project/ptv-tasks-administrator/backend/app/services/keycloak_service.py): 2-Tier Hybrid Keycloak (Admin REST API 300ms + RPA Fallback, cập nhật trạng thái tài khoản qua cờ boolean `enabled` trực tiếp).
11. [`session_keepalive_service.py`](file:///c:/Users/dtt/Desktop/Project/ptv-tasks-administrator/backend/app/services/session_keepalive_service.py): Dịch vụ Unified Session Keepalive v2.0 tuần tự gieo mầm (sequential seeding) và ping ngầm làm ấm phiên 7 cổng dịch vụ.
12. [`cron_telemetry.py`](file:///c:/Users/dtt/Desktop/Project/ptv-tasks-administrator/backend/app/core/cron_telemetry.py): Hệ thống theo dõi vi lượng trạng thái 7 background cronjobs, lưu vết độ trễ và đẩy lên Supabase `cron_telemetry_state`.

### 4.3. 14 Trang Chức Năng Frontend SPA (`frontend/src/features/`)
1. [`UnifiedInboxPage.tsx`](file:///c:/Users/dtt/Desktop/Project/ptv-tasks-administrator/frontend/src/features/inbox/UnifiedInboxPage.tsx): Trung tâm AI Workflow Console V3.1 (Bento Grid, Drawer 4 trạng thái, Trình xem trước tệp đính kèm đa định dạng SheetJS/PDF/Office/Images qua `AttachmentPreviewModal`, Modal điều khiển quy trình `WorkflowConsoleModal`, Duyệt luồng thực thi).
...
1. **Hermetic Pytest Suite:** `tests/test_capability_contracts.py`, `tests/test_execution_safety.py`, `tests/test_planning_policy.py`, `tests/test_request_fact_normalizer.py`, `tests/test_security_and_provenance.py`, `tests/test_workflow_legacy_replan.py`.
...
4. **Git Collaborator End-to-End Suite:** [`test_git_collaborator.py`](file:///c:/Users/dtt/Desktop/Project/ptv-tasks-administrator/backend/test_git_collaborator.py) - Kiểm tra đồng bộ nhiều kho Git và cơ chế bảo vệ bot admin.
